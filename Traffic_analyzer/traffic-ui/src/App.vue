...
const runtimeEventsOn = (event, cb) => window?.runtime?.EventsOn ? window.runtime.EventsOn(event, cb) : null;
const runtimeEventsOff = (event) => window?.runtime?.EventsOff ? window.runtime.EventsOff(event) : null;
...
function fitTerminal() {
  if (!terminalEmulator || !fitAddon || !hasDesktopBridge.value) return;
  fitAddon.fit();
...
async function hydrateTerminalHistory(id) {
  if (!id || !hasDesktopBridge.value) return;
...
function ensureTerminalEmulator() {
  if (!hasDesktopBridge.value || terminalEmulator || !terminalHostRef.value) return;
  ...
  terminalEmulator.onData((input) => {
    if (!activeTerminalId.value || !hasDesktopBridge.value) return;
...
function onTerminalStream(payload) {
  if (!hasDesktopBridge.value || !payload || !payload.id) return;
  try {
...
async function closeActiveTerminal() {
  if (!hasDesktopBridge.value || !activeTerminalId.value || activeTerminalId.value === "local-default") return;
...
async function reconnectActiveTerminal() {
  if (!hasDesktopBridge.value || !activeTerminalId.value) return;
  try {
...
async function copyActiveTerminalText() {
  ...
  if (!window?.runtime?.ClipboardSetText) {
    actionMessage.value = "当前环境不支持剪贴板复制";
    return;
  }
  try {
...
async function init() {
  if (hasDesktopBridge.value) {
    ...
    await refreshServiceStatus();
    await refreshTerminals();
    await refreshCurrentTerminalUser();
    await hydrateTerminalHistory(activeTerminalId.value);
    runtimeEventsOn("terminal:stream", onTerminalStream);
  } else {
...
onUnmounted(() => {
  stopPolling();
  runtimeEventsOff("terminal:stream");
  if (fitTimer) {
...
<template>
  <div class="app">
    ...
    <section class="terminal-center" v-show="terminalVisible && hasDesktopBridge">
